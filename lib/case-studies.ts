...
export const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    decisions: [
      { decision: "Architecture", choice: "4 dedicated proxy instances with role-based routing", alternatives: "Single proxy with config switching, API gateway", why: "Dedicated instances prevent a single-point failure from taking down all AI services. Each instance has a distinct role — main handles switchable routing, standby locks to Claude for reliability, A0 routes ChatGPT traffic for autonomous agents, and conversation provides zero-overhead passthrough." },
      { decision: "Cost tracking granularity", choice: "Per-request cost attribution with model-level breakdown", alternatives: "Monthly aggregate billing, per-team budgets", why: "Knowing exactly what each model costs per request enables intelligent routing decisions. The dashboard shows $62.45 broken down across claude-sonnet-4, gpt-4o, and Azure — enabling data-driven provider selection." },
      { decision: "Failover strategy", choice: "Health-check-driven automatic rerouting in milliseconds", alternatives: "Manual provider switching, DNS-based failover", why: "LLM providers have weekly outages. Manual switching means 5-15 minutes of downtime per event. Auto-failover with health polling detects failures in under a second and reroutes instantly." }
    ],
    phases: [
      { title: "Core proxy engine", description: "High-performance request routing with provider-specific adapters for Claude, GPT, and Azure APIs. Each proxy instance runs independently with its own health monitoring." },
      { title: "Multi-instance deployment", description: "4 dedicated instances across ports 3200-3203: main (switchable between providers), standby (Claude-dedicated), A0 (ChatGPT for autonomous agents), conversation (passthrough for real-time chat)." },
      { title: "Monitoring dashboard", description: "Real-time Next.js dashboard showing instance health, 24-hour request volume charts, per-model cost breakdown, latency comparison across providers, and failover event history." },
      { title: "AI cost optimization", description: "Integrated AI that analyzes request patterns across thousands of daily calls to recommend routing optimizations — identifying where cheaper models can handle tasks without quality loss." }
    ],
    challenges: [
      { problem: "LLM provider outages happen weekly", impact: "All AI services go down simultaneously, affecting every product", solution: "Auto-failover across 4 instances with sub-second health polling", result: "Zero downtime from provider issues — traffic reroutes before users notice" },
      { problem: "No visibility into AI spending", impact: "Monthly bills are unpredictable, impossible to attribute costs to features", solution: "Per-request cost tracking with model-level breakdown and trend analysis", result: "Full cost visibility — know exactly what each AI feature costs in real-time" },
      { problem: "Expensive models used for simple tasks", impact: "60% of requests could be handled by cheaper models at equal quality", solution: "AI-powered complexity classification routes simple queries to cost-effective models", result: "40% cost reduction while maintaining output quality on complex tasks" }
    ],
    aiFeatures: [
